...
pub mod quat_test {
    ...
    #[test]
    fn quat_to_mat() {
        let m0 = Mat3n::from(Quat::new(0.0, 1.0, 0.0, 0.0)); //rotation around x
    ...
    fn quat_rotation_order_verification() {
        ...
        let q1 = Quat::get_quat_euler_obj_upr_deg(45.0, 45.0, 45.0);
        assert_eq!(q0, q1);
    }
    ...
    #[test]
    fn quat_euler() {
        let input = Vec3n::new(75.2, -123.0, 62.0);
        let output = Quat::get_quat_euler_upr_obj_deg(input.x, input.y, input.z).get_euler_angles_upr_obj_deg();
        ...
        let input = Vec3n::new(90.0, 0.0, -60.0);
        let output = Quat::get_quat_euler_upr_obj_deg(input.x, input.y, input.z).get_euler_angles_upr_obj_deg();
        let delta = input - output;
        assert!(delta.length().abs() < 0.0001); //tested this way, because floating point error is hell
    }
