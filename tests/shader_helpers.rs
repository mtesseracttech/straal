...
impl Shader {
    ...
    pub const PHONG: Self = Self {
        vertex: include_str!("res/phong/phong.vert"),
        fragment: include_str!("res/phong/phong.frag"),
        geometry: None,
    };

    #[allow(dead_code)]
    pub const GOURAUD: Self = Self {
        vertex: include_str!("res/gouraud/gouraud.vert"),
        fragment: include_str!("res/gouraud/gouraud.frag"),
        geometry: None,
    };
    ...
    #[allow(dead_code)]
    pub fn load_glium_shader(display: &glium::Display, shader_type: Self) -> Option<glium::Program> {
        Shader::compile_glium_shader(display, shader_type.vertex, shader_type.fragment, shader_type.geometry)
    }

    #[allow(dead_code)]
    pub fn load_glium_shader_dynamic(display: &glium::Display, shader_name: &str) -> Option<glium::Program> {
        let folder_location = format!("shaders/{}/", shader_name);
        if !Shader::path_exists(folder_location.as_str()) {
            ...
            Shader::read_glium_shader(display, vert_shader_path.as_str(), frag_shader_path.as_str(), geom_shader)
        }
    }
    ...
    fn read_glium_shader(display: &glium::Display, vert_path: &str, frag_path: &str, geom_path: Option<&str>) -> Option<glium::Program> {
        ...
        Shader::compile_glium_shader(display, &vert_source, &frag_source, geom_source)
    }

    fn compile_glium_shader(display: &glium::Display, vert_src: &str, frag_src: &str, geom_src: Option<&str>) -> Option<glium::Program> {
