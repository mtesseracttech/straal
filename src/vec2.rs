...
impl<S> Vec2<S> where S: FloatType<S> {
    ...
    pub fn reflect_safe(i: Vec2<S>, n: Vec2<S>) -> Vec2<S>
    {
        let n = n.normalized();
        i - n * S::from(2).unwrap() * (i).dot(n)
    }

    pub fn refract(v: Vec2<S>, n: Vec2<S>, ni_over_nt: S) -> Option<Vec2<S>> {
    ...
    pub fn refract_safe(v: Vec2<S>, n: Vec2<S>, ni_over_nt: S) -> Option<Vec2<S>> {
        let n = n.normalized();
        let unit_v = v.normalized();
        let dt = unit_v.dot(n);
        let discriminant = S::one() - ni_over_nt * ni_over_nt * (S::one() - dt * dt);
