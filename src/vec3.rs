...
impl<S> Sum<Vec3<S>> for Vec3<S>
    where
        S: FloatType<S>,
{
    fn sum<I: Iterator<Item=Vec3<S>>>(iter: I) -> Self {
        iter.fold(Vec3::<S>::zero(), |a, b|
            a + b,
        )
    }
}

impl<'a, S> Sum<&'a Vec3<S>> for Vec3<S>
    where
        S: FloatType<S>,
{
    fn sum<I: Iterator<Item=&'a Vec3<S>>>(iter: I) -> Self {
        iter.fold(Vec3::<S>::zero(), |a, b|
            a + *b,
        )
    }
